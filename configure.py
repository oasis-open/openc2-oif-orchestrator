...
parser.add_option("-o", "--orc-gui", action="store_true", dest="orc_gui", default=False, help="Build Orchestrator GUI")
...
CONFIG = FrozenDict(
    ...
    ModuleCopy=FrozenDict(
        utils=(
            ...
            ("orchestrator", "transport", "mqtt", "module"),
        )
    ),
    GUIS=FrozenDict(
...
if __name__ == "__main__":
    ...
    if (not options.build_image and not options.orc_gui and not options.log_gui) or options.orc_gui:
        ...
        if build_gui(system, os.path.join(CONFIG.WorkDir, *CONFIG.GUIS.Orchestrator), Stylize):
