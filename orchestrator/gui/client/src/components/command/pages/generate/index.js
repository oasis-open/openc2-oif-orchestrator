...
class GenerateCommands extends Component {
  ...
  sendCommand() {
    if ('command_id' in this.state.message) {
      if (!validateUUID4(this.state.message.command_id)) {
        toast(<div><p>Error:</p><p>Command ID is not a valid UUIDv4</p></div>, {type: toast.TYPE.WARNING});
        return;
      }
    }
    ...
    toast(<div><p>Request sent</p></div>, {type: toast.TYPE.INFO});
