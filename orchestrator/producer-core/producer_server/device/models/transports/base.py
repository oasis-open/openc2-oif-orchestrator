...
class Transport(PolymorphicModel):
    ...
    def etcd_data(self):
        return {
            'host': self.host,
            'port': self.port,
            'serialization': json.dumps([s.name for s in self.serialization.all()])
        }

    def __str__(self):
        return f"{self.host}:{self.port} - {self.protocol.name}"
...
@receiver(m2m_changed, sender=Transport.serialization.through)
def etcd_m2m_save(sender, instance=None, **kwargs):
    key_base = f'/transport/{instance.protocol.name}/{instance.transport_id}'
    action = kwargs.get('action', None)
    if action in ("post_add", "post_remove"):
        vals = [s.name for s in instance.serialization.all()]
        settings.ETCD_CLIENT.write(f'{key_base}/serialization', json.dumps(vals))
    ...
    try:
        ...
        for key in {*db_data.keys(), *etcd_data.keys()}:
            ...
            if db_val and not e_key:
                settings.ETCD_CLIENT.write(f'{key_base}/{key}', json.dumps(db_val))
            elif e_key and not db_val:
                settings.ETCD_CLIENT.delete(f'{key_base}/{key}')
            elif instance.data_changed([key]):
                e_key.value = json.dumps(db_val)
                settings.ETCD_CLIENT.update(e_key)
    except etcd.EtcdKeyNotFound:
        for key, val in db_data.items():
            settings.ETCD_CLIENT.write(f'{key_base}/{key}', json.dumps(val))
